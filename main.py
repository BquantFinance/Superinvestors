...
# ═══════════════════════════════════════════════════════════════
# CSS — Refined dark terminal with warm accents
# ═══════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
/* ── Animated gradient strip at very top ── */
.gradient-strip {
    height: 3px;
...
<div class="header-row">
    <div>
        ...
        <div class="header-sub">Carteras de los mejores inversores del mundo · Análisis visual en tiempo real</div>
    </div>
...
n_stk = df['Stock'].nunique()
aum = df['Value_Clean'].sum() / 1e9
avg_pos = len(df) / n_inv if n_inv > 0 else 0
...
<div class="metrics-grid">
    ...
    <div class="m-card c-green">
        ...
        <div class="m-delta green">Billones USD</div>
    </div>
    <div class="m-card c-amber">
        <div class="m-label">Pos. Promedio</div>
        <div class="m-value">{avg_pos:.0f}</div>
        <div class="m-delta amber">Por inversor</div>
    </div>
    <div class="m-card c-red">
        <div class="m-label">Conc. Top 5</div>
        <div class="m-value">{avg_conc:.1f}%</div>
        <div class="m-delta red">Promedio</div>
    </div>
...
with c1:
    scope = st.radio("scope", ["Top 5", "Top 10", "Top 20", "Todos"], index=1, horizontal=True, label_visibility="collapsed")

with c2:
    if scope == "Top 5":
        defaults = df.groupby('Investor')['Value_Clean'].sum().nlargest(5).index.tolist()
    elif scope == "Top 10":
        defaults = df.groupby('Investor')['Value_Clean'].sum().nlargest(10).index.tolist()
    elif scope == "Top 20":
        defaults = df.groupby('Investor')['Value_Clean'].sum().nlargest(20).index.tolist()
    else:
        defaults = sorted(df['Investor'].unique())

    selected = st.multiselect("inv", sorted(df['Investor'].unique()), default=defaults, label_visibility="collapsed")

with c3:
    threshold = st.radio("thresh", ["Top 80%", "Top 90%", "Todas"], index=1, horizontal=True, label_visibility="collapsed")
...
# Activity score for color
act_score = {'Compra': 1.0, 'Añadir': 0.5, 'Mantener': 0.0, 'Reducir': -1.0}
treemap_df['Act_Score'] = treemap_df['Activity_Type'].map(act_score)
...
fig = px.treemap(
    ...
    color_continuous_scale=[
        [0.0,  '#dc2626'],   # Reduce — vivid red
        [0.35, '#1c1c2e'],   # Hold — near-black
        [0.65, '#3b82f6'],   # Add — electric blue
        [1.0,  '#22c55e'],   # Buy — vivid green
    ],
    range_color=[-1, 1],
...
fig.update_traces(
    hovertemplate=(
        ...
        '<span style="color:#9090a8">Actividad:</span> %{customdata[4]}'
        '<extra></extra>'
    ),
...
fig.update_layout(
    ...
    coloraxis_colorbar=dict(
        title=dict(text='ACTIVIDAD', font=dict(size=9, family='DM Mono, monospace', color='#505068')),
...
consensus = df[df['Investor'].isin(selected)].groupby('Ticker').agg(
    ...
    Total_Acciones=('Activity_Type', 'count'),
).reset_index()
...
consensus['Sentimiento'] = consensus.apply(
    lambda r: '🟢 Alcista' if r['Comprando'] > r['Total_Acciones'] * 0.5
...
with col_chart:
    ...
    fig_bar.add_trace(go.Bar(
        y=top15['Ticker'] + '  ' + top15['Empresa'].str[:18],
        x=top15['Inversores'],
        orientation='h',
        marker=dict(
        ...
        text=top15['Inversores'].astype(str),
        textposition='inside',
...
with col_table:
    display_df = consensus[['Ticker', 'Empresa', 'Inversores', 'Peso_Promedio', 'Peso_Max', 'Valor_M', 'Sentimiento']].copy()
    display_df.columns = ['Ticker', 'Empresa', 'Nº Inv.', 'Peso Medio %', 'Peso Máx %', 'Valor $M', 'Sentimiento']

    # FIX: cast numpy types to native Python for JSON serialization
    max_inv = int(display_df['Nº Inv.'].max()) if len(display_df) > 0 else 10
    ...
    st.dataframe(
        ...
        column_config={
            ...
            'Nº Inv.': st.column_config.ProgressColumn(
                min_value=0,
                max_value=max_inv,
                format='%d',
                width='small',
...
<div class="footer-bar">
    <div class="footer-text">
        ...
        Datos: Dataroma · {n_inv} inversores · {n_stk:,} acciones · Agosto 2025
    </div>
</div>
""", unsafe_allow_html=True)
